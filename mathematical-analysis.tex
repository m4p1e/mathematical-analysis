...
\begin{definition}
单调函数分为广义的单调函数和严格的单调函数. 例如当$x > x'$有$f(x) \geq f(x')$就是广义单调增函数，也叫不减函数(菲砖). 与之对应的严格单调增函数就需要把前面这个不等式的等号去掉.
\end{definition}


\begin{theorem}
设$f(x)$在区域$\mathcal{X}$内单调增加，即使是广义的也可以. 区域$X$以大于一切$x$的数$a$(它可以是有限的，也可以是$-\infty$)作为聚点. 若在这时$f(x)$上有界:
$$
